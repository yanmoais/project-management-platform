...
async def test_BD(browser_args):
    """
    为 test_BD 创建完全独立的浏览器实例，使用指定的浏览器参数
    """
    task_id = "test_BD"
    async with async_playwright() as p:
        ...
        try:
            ...
            if await ui_operations.is_browser_closed():
                log_info(f"[{task_id}] 检测到浏览器已关闭，test_BD 测试无法继续")
                raise Exception("BROWSER_CLOSED_BY_USER")

            # 测试步骤1: 输入搜素内容 (操作次数: 1)
            with allure.step("测试步骤1: 输入搜素内容"):
                log_info(f"[{task_id}] 开始测试步骤1 输入搜素内容 的操作==============")

                # 公共断言方法，断言元素是否存在
                with allure.step("测试步骤1: 公共断言元素是否存在"):
                    await ui_operations.elem_assert_exists("//*[@id='chat-textarea']")
                ...
                for attempt in range(1):
                    ...
                    try:
                        ...
                        await ui_operations.elem_input("//*[@id='chat-textarea']", "AI测试哪个好用")
                        time.sleep(1)
            ...
            # 测试步骤2: 点击搜索按钮 (操作次数: 1)
            with allure.step("测试步骤2: 点击搜索按钮"):
                log_info(f"[{task_id}] 开始测试步骤2 点击搜索按钮 的操作==============")
...
async def test_concurrent_independent_browsers():
    ...
    try:
        ...
        # Create tasks
        task1 = asyncio.create_task(test_BD(browser_args_list[0]))
