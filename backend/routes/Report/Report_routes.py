...
def get_product_packages():
    ...
    try:
        ...
        packages = db.session.query(Project.product_package_name).distinct().all()
        package_list = [p[0] for p in packages if p[0]]
        return jsonify({'code': 200, 'msg': 'success', 'data': package_list})
    except Exception as e:
...
def generate_report():
    ...
    try:
        ...
        if not product_packages:
            return jsonify({'code': 400, 'msg': 'Please select at least one product'})
        ...
        selected_projects = Project.query.filter(Project.product_package_name.in_(product_packages)).all()
        ...
        # Logic Change: Match AutomationProject.project_id to Project.id
        relevant_aps = AutomationProject.query.filter(
            AutomationProject.project_id.in_(project_map.keys())
        ).all()
        ...
        executions = []
        if relevant_ap_ids:
            executions = AutomationExecution.query.filter(
            ...
            ).order_by(AutomationExecution.start_time.desc()).all()
        ...
        # Calculate New Cases
        new_cases_count = AutomationProject.query.filter(
...
def render_log_html_pycharm_style(parsed_log, execution_id):
    ...
    <div class="log-header">
        ...
        <div class="stat-item">
            ...
            <div class="stat-value">{sum(len(step.get('logs', [])) for step in parsed_log.get('testSteps', [])) + len(parsed_log.get('initLogs', [])) + len(parsed_log.get('endLogs', []))}</div>
        </div>
    </div>
    """
...
<head>
    ...
    <script>
        ...
        function toggleProductsSection() {{
            ...
            const icon = document.getElementById('productsIcon');
            if (body.classList.contains('hidden')) {{
                body.classList.remove('hidden');
                icon.style.transform = 'rotate(0deg)';
            }} else {{
        ...
        function toggleExecution(id) {{
            const body = document.getElementById('exec-body-' + id);
            const icon = document.getElementById('exec-icon-' + id);
            if (body.classList.contains('hidden')) {{
                body.classList.remove('hidden');
                icon.style.transform = 'rotate(0deg)';
            }} else {{
        ...
        function showImage(path) {{
            // Normalize path (in case of slashes difference, though we used raw string as key)
            // But usually we just use the raw path string as key.
            
            const modal = document.getElementById("imageModal");
            const modalImg = document.getElementById("img01");
            const captionText = document.getElementById("caption");
            
            // Try to find image in embedded data
            // Note: path passed here might have escaped backslashes from format_log_line
            // In format_log_line we did: js_path = path.replace('\\', '\\\\')
            // So 'path' argument here is the original path string (JavaScript unescapes it when passing string literal)
            // Let's verify.
            
            const imageData = embeddedImages[path];
            
            if (imageData) {{
                modal.style.display = "block";
                modalImg.src = imageData;
                captionText.innerHTML = "Image Preview<br><div class='image-path-text'>" + path + "</div>";
            }} else {{
                alert("Image data not found embedded in this report for path:\\n" + path + "\\n\\nThe file might have been moved or deleted before report generation.");
            }}
        }}
        ...
        function initPagination() {{
            const containers = document.querySelectorAll('.paginated-container');
            containers.forEach(container => {{
                const items = Array.from(container.children).filter(el => el.classList.contains('execution-item'));
                ...
                function renderPage(page) {{
                    ...
                    if (controls) {{
                        controls.innerHTML = `
                            <button class="pagination-btn ${{page === 1 ? 'disabled' : ''}}" onclick="changePage(this, ${{page - 1}})">上一页</button>
                            <span class="pagination-info">第 ${{page}} / ${{totalPages}} 页 (共 ${{totalItems}} 条)</span>
                            <button class="pagination-btn ${{page === totalPages ? 'disabled' : ''}}" onclick="changePage(this, ${{page + 1}})">下一页</button>
                        `;
                    }}
                }}

                // Attach render function to container for access via global handler
        ...
        window.changePage = function(btn, newPage) {{
            const container = btn.closest('.paginated-container');
            if (container && container.renderPage) {{
    ...
    <div class="container">
        ...
        <div class="report-header">
            ...
            <div class="header-right">
                ...
                <div class="report-time">{current_time}</div>
            </div>
        </div>
        
        <!-- Products Section -->
        <div class="products-section">
            <div class="products-header" onclick="toggleProductsSection()">
                <span>涉及产品 ({len(packages)})</span>
                <i class="arrow-icon" id="productsIcon">▼</i>
            </div>
            <div class="products-body" id="productsBody">
                {''.join([f'<span class="product-tag">{p}</span>' for p in packages])}
            </div>
        </div>
        ...
        <div class="metrics-section">
            <div class="metrics-grid">
                <div class="metric-card">
                    <div class="metric-value">{stats.get('test_case_count', 0)}</div>
                ...
                <div class="metric-card">
                    <div class="metric-value success">{stats['success_count']}</div>
                    <div class="metric-label">成功数</div>
                </div>
                <div class="metric-card">
                    <div class="metric-value fail">{stats['fail_count']}</div>
