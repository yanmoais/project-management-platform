...
'''
        ...
        while i < len(test_steps):
            step = test_steps[i]
            # Check if this is a captcha step with retry enabled
            is_captcha_retry = step.get('operation_event') == 'solve_captcha' and str(step.get('captcha_retry_enabled', 'no')).lower() == 'yes'
            
            if is_captcha_retry and i + 1 < len(test_steps):
                next_step = test_steps[i+1]
                method_code += TestCodeGenerator._generate_captcha_retry_step_code(i + 1, step, next_step, safe_pid, product_address)
                i += 2 # Skip next step
                continue
            
            method_code += TestCodeGenerator._generate_step_code(i + 1, step, safe_pid, product_address)
            i += 1
...
'''
    ...
    @staticmethod
    def _generate_step_code(index, step, safe_pid, product_address):
        step_name = step.get('step_name', f'step_{index}')
...
'''
        ...
        if tab_switch_enabled == 'yes' and tab_target_url:
            code += f'''                
                # 标签页跳转配置
                ...
                # 公共断言方法，断言URL是否存在
                with allure.step("[{{task_id}}] 测试步骤{index}: 公共断言URL是否存在"):
...
'''
        ...
        if operation_type == 'web':
            if operation_event in ['input', 'select_option', 'press_key', 'drag_and_drop']:
                ...
                # operation_params 是验证码图片元素，input_value 是输入框元素
                invoke = f'await ui_operations.elem_solve_captcha("{operation_params}", "{input_value}")'
            else:
                # Click, etc.
