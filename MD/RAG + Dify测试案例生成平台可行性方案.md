...
- **用户交互界面**：基于 Vue 3 + Element Plus 的现代化 Web 界面
...
- **测试用例库**：使用 MySQL 存储结构化测试用例（与现有系统一致）
...
- **数据库**：MySQL + Redis（保持现有架构）

- **向量数据库**：Chroma（开发）/ Milvus（生产）

- **消息队列**：Celery + Redis（保持现有架构）
...
- **CI/CD**：GitLab CI / 自建流水线

### 与现有项目对齐要点

1. **后端结构对齐**：按 FastAPI 分层结构组织（`app/api`、`app/services`、`app/models`、`app/schemas`），逐步替换现有 Blueprint。
2. **权限模型对齐**：沿用现有 RBAC（用户-角色-菜单/按钮），生成平台的功能入口需绑定权限标识。
3. **数据库对齐**：新增表结构追加到 `create.sql`，避免覆盖；连接配置与现有 `settings.js` 保持一致。
4. **异步任务对齐**：长耗时任务（向量化、批量生成、导出）接入 Celery + Redis。
5. **前端结构对齐**：新增页面放在 `src/views`，样式放在 `src/assets/css`，路由注册在 `src/router`。
6. **接口调用对齐**：前端 API 继续集中在 `src/api` 目录统一封装。
...
【测试用例模板，需要解析上传的xmind模板文件来生成测试用例】
用例名称：
页面功能：
业务流程：
接口验证：
...
- **权限隔离**：知识库、数据集、用例库按角色与项目维度隔离

- **数据脱敏**：上传前自动识别并脱敏账号、手机号、密钥等信息

- **访问审计**：记录文档上传、检索调用、用例导出、删除等行为

#### Prompt 安全与模型防护

- **输入净化**：对用户输入和检索结果进行安全过滤

- **指令隔离**：系统提示与用户提示强隔离，避免被覆盖

- **模型输出校验**：对输出结构进行格式校验与修复

### 7. 评估与成本控制

#### 评估指标体系

- **覆盖率**：需求点覆盖率、接口路径覆盖率、边界场景覆盖率

- **有效性**：用例可执行率、失败可定位率、冗余率

- **一致性**：同类需求输出稳定性、回归用例变更稳定性

#### 成本与性能策略

- **请求合并**：同批需求合并检索与生成，减少模型调用次数

- **缓存复用**：向量检索结果与生成草稿缓存复用
