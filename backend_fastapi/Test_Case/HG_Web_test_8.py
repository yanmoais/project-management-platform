...
async def test_HG(browser_args):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 测试步骤1: 商户管理菜单页面不存在断言 (操作次数: 1)
            with allure.step("测试步骤1: 商户管理菜单页面不存在断言"):
                log_info(f"[{task_id}] 开始测试步骤1 商户管理菜单页面不存在断言 的操作==============")

                # 断言元素不存在
                with allure.step(f"[{task_id}] 测试步骤1: 断言元素不存在 {operation_params}"):
                    log_info(f"[{task_id}] 执行断言元素不存在: {operation_params}")
                    await ui_operations.elem_assert_not_exists("//*[@class='menu-title' and text()='商户管理']")
            ...
            # 测试步骤2: 商户管理菜单页面不存在断言1 (操作次数: 1)
            with allure.step("测试步骤2: 商户管理菜单页面不存在断言1"):
                log_info(f"[{task_id}] 开始测试步骤2 商户管理菜单页面不存在断言1 的操作==============")

                # 断言元素不存在
                with allure.step(f"[{task_id}] 测试步骤2: 断言元素不存在 {operation_params}"):
                    log_info(f"[{task_id}] 执行断言元素不存在: {operation_params}")
                    await ui_operations.elem_assert_not_exists("//*[@class='menu-title' and text()='商户管理']")
            ...
            # --- 开始执行重复步骤 (源步骤: 重复) ---
            with allure.step("测试步骤3: 重复 (重复步骤)"):
                log_info(f"[{task_id}] 开始测试步骤3 重复 (重复步骤) 的操作==============")
                ...
                log_info(f"[{task_id}] 重复步骤 重复 (重复步骤) 执行完成")
            # --- 结束执行重复步骤 ---
